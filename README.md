...
| POST   | /api/v1/users                                             | Bearer token, admin only              | email, full_name, role (staff roles only)                                     | Admin provisions a staff account; user sets their own password via reset flow. **Student accounts are not created here** — use `POST /api/v1/students`. |
...
| POST   | /api/v1/students                                          | Bearer token, registrar/admin         | email, full_name, program_id?, admission_term_id?                             | Creates a student account **and** profile together (auto-generated `student_id`)                                                                        |
